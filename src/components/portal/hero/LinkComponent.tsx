  ...
  if (item.children && item.children.length) {
          ...
          {({ open }) => (
            <>
              ...
              {open && (
                <Menu.Items
                  static
                  className="absolute z-10 mt-12 flex w-max flex-col items-center rounded-md bg-[#000000e3] tracking-wide text-white smHero:ml-80 smHero:mt-0 smHero:items-start"
                >
                  {item.children?.map((child) => (
                    <Menu.Item key={child.href}>
                      <Link
    ...
    return (
      <Link
        key={item.href}
        href={item.href}
        className={`flex w-min flex-col rounded-md px-4 py-2 text-center text-base/4 tracking-wide text-white ${
          isActive ? "bg-neutral-900" : "hover:bg-neutral-900"
        }`}
      >
        {item.name}
